(function install(root, factory) {
  ...
  /**
   * Taste's observed ratings payload calls its displayed IMDb aggregate `externalRating`, with
   * `average` and `total` values. Fail blank on any other shape: false identity evidence is
   * worse than missing evidence, and the profile owner's 1–4 rating must never leak in here.
   */
  function imdbEvidence(item) {
    const external = item.externalRating;
    if (!external || typeof external !== "object" || Array.isArray(external)) {
      return { rating: "", count: "" };
    }
    const rating = numericValue(external.average);
    const count = numericValue(external.total);
    return {
